...
const onSave = async () => {
  ...
  const ok = await hanchansApi.updateScores(hanchan.value.id, playedDate.value, results)
  saving.value = false
  if (!ok) {
...
const onDelete = async () => {
  ...
  const ok = await hanchansApi.deleteHanchan(hanchan.value.id)
  if (!ok) {
